<div class="createPost hidden absolute w-full h-full bg-black/80 flex justify-center items-start">
    <form method="POST" action="{{ route('createPost') }}" class="relative bg-white p-10 flex flex-col items-center mt-10 gap-5 w-[30%] rounded-xl">
        ...
        <input name="title" type="text" placeholder="Название записи"
               class="w-full border-[1px] border-black px-3 py-1 rounded-lg text-xl">
        <textarea name="content" placeholder="Контент записи"
        ...
        <select name="tags[]" multiple class="w-full border-[1px] border-black px-3 py-1 rounded-lg text-xl">
            @forelse($tags as $tag)
